...
def _calendar_connector(
    *,
    ...
    return GoogleReadConnector(
        configured_identity="operator.test",
        credential_store=credential_store,
        provider=ControlledGoogleReadProvider(
            result=result
            or GoogleReadProviderResult(
                items=(
                    json.dumps(
                        {
        ...
        trace=DiagnosticTraceRecorder(
            ...
            clock=clock,
            ids=DeterministicIdGenerator("ticket14-google-trace"),
        ),
        clock=clock,
        ids=DeterministicIdGenerator("ticket14-google-audit"),
...
def _calendar_adapter(
    ...
    *,
    ...
    after_reads: Callable[[], None] | None = None,
) -> AgentsSdkOrchestrationAdapter:
    calendar_connector = connector or _calendar_connector()

    def run_sync(agent: object, _text: str, **_kwargs: object) -> object:
        tools = agent.tools
        calendar_tool = next(
            tool for tool in tools if tool.name == "read_google_calendar"
        )
    ...
    return AgentsSdkOrchestrationAdapter(
        ...
        google_read_connector=calendar_connector,
    )
...
def test_calendar_insert_rejects_primary_without_freezing() -> None:
    complete_event = {
        "summary": "Design review",
        "start": {"dateTime": "2026-08-10T10:00:00Z"},
        "end": {"dateTime": "2026-08-10T11:00:00Z"},
    ...
    with pytest.raises(OrchestrationAdapterError, match="primary"):
        adapter.run(_request("create a design review"))


def test_calendar_insert_event_wrapper_is_normalized_before_freezing() -> None:
    complete_event = {
        "summary": "Design review",
        "start": {"dateTime": "2026-08-10T10:00:00Z"},
        "end": {"dateTime": "2026-08-10T11:00:00Z"},
    ...
    assert payload["calendar_id"] == "secondary-calendar"
    assert payload["complete_event"] == complete_event
    assert payload["notification"] == "none"
    assert payload["connection_generation"] == 1


def test_calendar_insert_uses_only_the_exact_returned_secondary_and_generation() -> (
    None
):
    complete_event = {
        "summary": "Design review",
    ...
    assert json.loads(result.proposal.payload)["connection_generation"] == 1


@pytest.mark.parametrize("calendar_id", ("unknown-calendar", "another-calendar"))
def test_calendar_insert_rejects_unknown_or_substituted_calendar(
    calendar_id: str,
) -> None:
    complete_event = {
        "summary": "Design review",
    ...
    with pytest.raises(OrchestrationAdapterError, match="grounded"):
        adapter.run(_request("create a design review"))


def test_calendar_insert_accepts_returned_secondary_among_other_secondaries() -> None:
    connector = _calendar_connector(
        result=GoogleReadProviderResult(
            items=(
                json.dumps({"id": "secondary-calendar", "primary": False}),
                json.dumps({"id": "another-calendar", "primary": False}),
            )
        )
    )
    adapter = _calendar_adapter(
...
def test_calendar_insert_rejects_reconnected_generation_before_freezing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connector = _calendar_connector()
    monkeypatch.setattr(connector, "current_connection_generation", lambda: 2)
    adapter = _calendar_adapter(
        AgentsSdkProposal(
            kind="calendar_insert",
            preview="Create the event.",
    ...
    with pytest.raises(OrchestrationAdapterError, match="stale"):
        adapter.run(_request("create a design review"))


def test_calendar_insert_rejects_read_without_connector_provenance() -> None:
    class MissingProvenanceConnector:
        def __getattr__(self, _name: str) -> object:
            return lambda **_kwargs: SimpleNamespace(
                service="calendar",
                operation="calendar_list",
                items=(json.dumps({"id": "secondary-calendar", "primary": False}),),
                truncated=False,
    ...
    def run_sync(agent: object, _text: str, **_kwargs: object) -> object:
        asyncio.run(
            agent.tools[0].on_invoke_tool(
                None,
                json.dumps({"operation": "calendar_list"}),
            )
        )
        return SimpleNamespace(final_output=AgentsSdkPlan(reply_text="unreachable"))
    ...
    adapter = AgentsSdkOrchestrationAdapter(
        ...
        reasoning_factory=_FakeReasoning,
        run_config_factory=_FakeRunConfig,
        google_read_connector=MissingProvenanceConnector(),
    )
...
@pytest.mark.parametrize(
    ("kind", "change_field"),
    (("calendar_update", "changes"), ("calendar_patch", "reviewed_patch")),
)
def test_calendar_change_freezes_only_an_observed_event_and_etag(
    kind: str,
    change_field: str,
) -> None:
    observed_event = {
    ...
    connector = _calendar_connector(result=result)
    payload = {
        "calendar_id": "secondary-calendar",
        ...
        change_field: {"summary": "Changed review"},
        "notification": "none",
    }
    adapter = _calendar_adapter(
        AgentsSdkProposal(kind=kind, preview="Change the event.", payload=payload),
        connector=connector,
        read_inputs=(
            {"operation": "calendar_list", "max_results": 50},
            {
...
def test_calendar_insert_preserves_a_valid_optional_event_id() -> None:
    complete_event = {
        "summary": "Design review",
        "start": {"dateTime": "2026-08-10T10:00:00Z"},
        "end": {"dateTime": "2026-08-10T11:00:00Z"},
    ...
    assert result.proposal is not None
    assert json.loads(result.proposal.payload)["event_id"] == "event-ticket31"


def test_calendar_insert_event_id_does_not_ground_an_unreviewed_calendar() -> None:
    complete_event = {
        "summary": "Design review",
        "start": {"dateTime": "2026-08-10T10:00:00Z"},
        "end": {"dateTime": "2026-08-10T11:00:00Z"},
    ...
    adapter = _calendar_adapter(
        AgentsSdkProposal(
            ...
            preview="Create the event.",
            payload={
                "calendar_id": "unreviewed-calendar",
                "complete_event": complete_event,
                "notification": "none",
                "event_id": "event-ticket31",
            },
...
@pytest.mark.parametrize("event_id", ("", " event-ticket31 ", 31))
def test_calendar_insert_rejects_invalid_optional_event_id(event_id: object) -> None:
    complete_event = {
        "summary": "Design review",
        "start": {"dateTime": "2026-08-10T10:00:00Z"},
        "end": {"dateTime": "2026-08-10T11:00:00Z"},
    ...
    with pytest.raises(OrchestrationAdapterError, match="malformed"):
        adapter.run(_request("create a design review"))
...
def test_calendar_insert_normalization_rejects_duplicate_or_untrusted_fields(
