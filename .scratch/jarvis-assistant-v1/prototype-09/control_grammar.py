...
REJECTION_PHRASES = frozenset(
    {"4", "no", "reject", "deny", "cancel", "cancel action", "don't do it", "do not do it"}
)
...
def _handle_command(state: ControlState, message: str) -> Transition:
    ...
    if command == "/new" and not args:
        ...
        return Transition(updated, (f"Started {updated.session_id} with persistent defaults. Previous work will not resume.",), ("cancel_and_new",))
    ...
    if command == "/cancel" and not args:
        ...
        updated = replace(state, active_request=None, pending_action=None, last_notice="Active work cancelled; pending action invalidated.")
        return Transition(updated, ("Cancelled the active request and invalidated its pending action. No side effect will start.",), ("cancel_request",))
    if command == "/model":
        if not args:
            return Transition(state, (f"Session model: {state.model}. Valid: {', '.join(MODELS)}.",))
        if len(args) != 1 or args[0] not in MODELS:
            return Transition(state, (f"Invalid model. Use exactly one of: {', '.join(MODELS)}.",))
        if state.active_request:
            return Transition(state, ("Model cannot change while a request or approval is active. Cancel or finish it first.",))
        updated = replace(state, model=args[0], last_notice=f"Session model set to {args[0]}.")
        return Transition(updated, (f"Session model set to {args[0]}. Persistent default unchanged.",))
    if command == "/reasoning":
        if not args:
            return Transition(state, (f"Session reasoning: {state.reasoning}. Valid: {', '.join(REASONING_LEVELS)}.",))
        if len(args) != 1 or args[0] not in REASONING_LEVELS:
            return Transition(state, (f"Invalid reasoning. Use exactly one of: {', '.join(REASONING_LEVELS)}.",))
        if state.active_request:
            return Transition(state, ("Reasoning cannot change while a request or approval is active. Cancel or finish it first.",))
        updated = replace(state, reasoning=args[0], last_notice=f"Session reasoning set to {args[0]}.")
        return Transition(updated, (f"Session reasoning set to {args[0]}. Persistent default unchanged.",))
    if command == "/config":
    ...
    if command == "/permissions" and not args:
        ...
        lines.extend(f"{p.id}: {p.lifetime}; {p.host}; `{p.command}`; cwd `{p.cwd}`" for p in state.permissions)
...
def _handle_config(state: ControlState, args: list[str]) -> Transition:
    ...
    if state.active_request:
        return Transition(state, ("Persistent configuration cannot change while a request or approval is active. Cancel or finish it first.",))
    if len(args) != 2:
        return Transition(state, ("Usage: /config model VALUE, /config reasoning VALUE, or /config session-minutes VALUE.",))
    key, value = args
    if key == "model" and value in MODELS:
        return Transition(replace(state, default_model=value), (f"Persistent model default set to {value}; current session remains {state.model}.",))
    if key == "reasoning" and value in REASONING_LEVELS:
        return Transition(replace(state, default_reasoning=value), (f"Persistent reasoning default set to {value}; current session remains {state.reasoning}.",))
    if key == "session-minutes" and value.isdigit() and int(value) in SESSION_MINUTES:
        minutes = int(value)
        return Transition(replace(state, session_minutes=minutes), (f"Working-session inactivity boundary set to {minutes} minutes, effective now and for future sessions.",))
    return Transition(state, ("Invalid config value. Use canonical models/reasoning or session-minutes 15, 30, 60, 120, or 240.",))


def _handle_revoke(state: ControlState, args: list[str]) -> Transition:
    if len(args) != 1:
        return Transition(state, ("Usage: /revoke <permission ID|session|persistent|all>.",))
    selector = args[0]
    ...
    updated = replace(state, permissions=after, last_notice=f"Revoked {removed} permission(s).")
    return Transition(updated, (f"Revoked {removed} permission(s) immediately.",), ("revoke_permission",))
...
def _resolve_approval(state: ControlState, choice: ApprovalChoice) -> Transition:
    ...
    if choice == "reject":
        updated = replace(state, pending_action=None, active_request=None, last_notice=f"Rejected {action.id}.")
        return Transition(updated, (f"Rejected {action.id}. The frozen payload was removed and nothing executed.",), ("reject_action",))

    if choice in {"session", "persistent"} and not action.permission_eligible:
        return Transition(state, ("That action permits only ‘Allow this time’. Reply 1 to allow once or 4 to reject.",))
    ...
    if choice in {"session", "persistent"}:
        ...
        permission = Permission(f"P-{next_permission:03d}", choice, action.host, action.command, action.cwd)
    ...
    updated = replace(
        ...
        active_request=replace(state.active_request, phase="executing") if state.active_request else None,
        last_notice=f"Approved {action.id} ({choice}).",
    )
    permission_text = f" Created {permission.id} ({permission.lifetime})." if permission else ""
    return Transition(updated, (f"Approved exact action {action.id} {choice.replace('_', ' ')}.{permission_text} Executing now.",), ("execute_action",))
...
def handle_system_event(state: ControlState, event: str, *args: str) -> Transition:
    ...
    if event == "route":
        if not state.active_request or state.pending_action or state.active_request.phase != "routing":
            return Transition(
                state,
        ...
        if not reason:
            reason = "Ubuntu default; no personal-laptop dependency detected" if host == "ubuntu" else "request requires the personal Windows laptop"
        ...
        label = "your personal Windows laptop" if host == "windows" else "Ubuntu"
        updated = replace(state, active_request=request, last_notice=f"Agent routed {request.id} to {host}: {reason}.")
        return Transition(
            updated,
    ...
    if event == "milestone":
        if not state.active_request or state.pending_action or not state.active_request.host:
            return Transition(state, ("Prototype milestone refused: enter a request and simulate its agent routing decision first.",))
        text = " ".join(args) or "Working"
        request = replace(state.active_request, phase=text.lower().replace(" ", "_"))
        return Transition(replace(state, active_request=request), (f"{request.id} update: {text}.",))
    if event == "propose":
        if not state.active_request or state.pending_action or not state.active_request.host:
            return Transition(
                state,
        ...
        command = "Restart-Service -Name ExampleService" if is_windows else "systemctl restart example.service"
        ...
        choices = "1 Allow this time | 4 Reject" if mandatory else "1 Allow this time | 2 Allow for this session | 3 Allow every time | 4 Reject"
        updated = replace(
            state,
    ...
    if event == "complete":
        ...
        return Transition(replace(state, active_request=None, pending_action=None, last_notice=f"Completed {request_id}."), (f"{request_id} completed successfully. Final result: simulated work finished.",), ("finish_request",))
    if event == "expire":
        ...
        return Transition(replace(state, pending_action=None, active_request=None, last_notice=f"Expired {action_id}."), (f"{action_id} expired after 10 minutes. Nothing executed; ask again for a fresh proposal.",), ("expire_action",))
    if event == "restart":
        persistent = tuple(p for p in state.permissions if p.lifetime == "persistent")
        updated = replace(state, active_request=None, pending_action=None, permissions=persistent, last_notice="Service restarted; active work interrupted and session permissions revoked.")
        return Transition(updated, ("Jarvis restarted. Active work will not resume; pending approval is invalid; persistent permissions remain.",), ("restart",))
    if event == "availability" and len(args) == 2 and args[0] in HOSTS and args[1] in {"ready", "down"}:
        field = f"{args[0]}_ready"
        updated = replace(state, **{field: args[1] == "ready"})
        return Transition(updated, (f"Prototype: {args[0].title()} is now {'ready' if args[1] == 'ready' else 'unavailable'}.",))
