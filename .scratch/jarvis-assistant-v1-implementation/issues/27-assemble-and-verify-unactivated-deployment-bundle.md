...
- Implemented on `codex/ticket-27-deployment-bundle` as a manual-activation-profile Compose bundle with distinct service identities, least-privilege container boundaries, pinned build inputs, private trust-segment networks, exact resource ceilings, service-specific credential mounts, and an offline adverse-case verifier. OpenWA and both native workers remain outside the Compose project; no service, credential, firewall, production network, or live dependency was activated during verification.
