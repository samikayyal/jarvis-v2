...
- PR #16 third review repair: the deployed working-session store is serialized and cross-thread safe; valid escaped terminal envelopes fit the fixed protocol bound; exact network membership is verified for every role; webhook responses end after durable admission while background ingress continues; Codex retains only reviewed proxy routing; orchestration cancellation crosses the authenticated service seam; Agents SDK model turns are cancelled at their configured deadline; and correlated Windows session calls let cancellation terminate an active Job Object execution.
