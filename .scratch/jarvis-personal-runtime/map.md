...
- [Retire the legacy runtime and finish](issues/13-retire-legacy-and-finish.md) — After separate operator authorization, remove the obsolete control-plane source, tests, dependencies, deployment assets, Ubuntu services, Compose stack, and Windows worker; retain OpenWA and project history; rewrite active guidance for the native personal runtime; and prove the surviving suite plus post-retirement live state.
